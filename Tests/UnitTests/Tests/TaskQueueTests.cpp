        ...
        for (size_t t = 0; t < threadCount; ++t)
        {
            threads.emplace_back([&]
            {
                ...
                for (size_t iter = 0; iter < iterationsPerThread; ++iter)
                {
                    ...
                    // CRITICAL: Terminate with wait=false so OnTerminationCallback runs async
                    // This is where the race occurs: OnTerminationCallback will call nested Terminate()
                    // while other threads are also in SignalTerminations() or ScheduleTermination()
                    hr = XTaskQueueTerminate(delegateQueue, avoidRaceMode, nullptr, nullptr);
